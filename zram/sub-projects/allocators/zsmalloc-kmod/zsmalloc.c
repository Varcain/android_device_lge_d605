...
struct zs_pool *zs_create_pool(const char *name, gfp_t flags)
{
	...
	for (i = 0; i < ZS_SIZE_CLASSES; i++) {
		...
		class->vm = alloc_vm_area(2 * PAGE_SIZE, class->vm_ptes);
		if (!class->vm) {
			error = -ENOMEM;
			goto cleanup;
		}
...
void *zs_malloc(struct zs_pool *pool, size_t size)
{
	...
	memset(link, POISON_INUSE, sizeof(*link)); // XXX: DEBUG
	kunmap_atomic(link);
...
void *zs_map_object(struct zs_pool *pool, void *handle)
{
	...
	if (off + class->size <= PAGE_SIZE) {
		...
		class->vm_addr = kmap_atomic(page);
	} else {
		...
		spin_lock(&class->lock);
		set_pte(class->vm_ptes[0], mk_pte(page, PAGE_KERNEL));
		set_pte(class->vm_ptes[1], mk_pte(nextp, PAGE_KERNEL));

		//ret = map_kernel_range_noflush((unsigned long)class->vm->addr,
		//				2 * PAGE_SIZE, PAGE_KERNEL,
		//				pages);
		/* We pre-allocated VM area so mapping can never fail */
		//BUG_ON(ret != 2);
		class->vm_addr = class->vm->addr;
	}

	return class->vm_addr + off;
}
...
void zs_unmap_object(struct zs_pool *pool, void *handle)
{
	...
	if (off + class->size <= PAGE_SIZE) {
		kunmap_atomic(class->vm_addr);
	} else {
		set_pte(class->vm_ptes[0], (pte_t) { 0 });
		set_pte(class->vm_ptes[1], (pte_t) { 0 });
		__flush_tlb_one((unsigned long)class->vm_addr);
		__flush_tlb_one((unsigned long)class->vm_addr + PAGE_SIZE);
...
// XXX: Remove when merged into kernel
